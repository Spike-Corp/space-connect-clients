...
public class StreamView extends SurfaceView {
    ...
    // Ported from Artemis: when true, onMeasure() picks the aspect-ratio-preserving size that
    // COVERS the whole available space (may overflow/crop one dimension) instead of the one
    // that FITS within it (letterbox/pillarbox).
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (fillDisplay) {
            // Ported from Artemis: pick the size that COVERS the available space (inverted
            // comparison vs. the FIT branch below), which will overflow/crop one dimension.
