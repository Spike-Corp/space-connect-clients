...
import com.limelight.binding.audio.MicForwarder;
...
public class Game extends Activity implements SurfaceHolder.Callback,
    ...
    private MicForwarder micForwarder;
    ...
    // Ported from Artemis: in-stream quick menu + pan/zoom + rotate-screen support.
    private GameMenu gameMenu;
    private boolean isPanZoomMode = false;
    private com.limelight.utils.PanZoomHandler panZoomHandler;
    private int currentOrientation;

    // Ported from Artemis: external/secondary display streaming support.
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Ported from Artemis: track the current orientation for rotateScreen()/auto-orientation
        // support. Defaults to landscape unless the user has opted into auto-orientation, in
        // which case we start with whatever the device is currently held as.
        ...
        if (prefConfig.videoScaleMode == PreferenceConfiguration.ScaleMode.STRETCH || shouldIgnoreInsetsForResolution(prefConfig.width, prefConfig.height)) {
        ...
        // Ported from Artemis: in-stream quick menu (rotate screen, pan/zoom toggle, quit, etc.)
        gameMenu = new GameMenu(this);
        findViewById(R.id.gameMenuButton).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                gameMenu.showMenu();
            }
        ...
        // Ported from Artemis: pinch-to-zoom/pan handler for the stream view, toggled on/off
        // via the game menu (see isPanZoomMode/toggleZoomMode()).
        panZoomHandler = new com.limelight.utils.PanZoomHandler(getApplicationContext(), this, streamView, prefConfig);

        if (prefConfig.alignDisplayTopCenter) {
            FrameLayout.LayoutParams streamViewParams = (FrameLayout.LayoutParams) streamView.getLayoutParams();
            streamViewParams.gravity = Gravity.CENTER_HORIZONTAL | Gravity.TOP;
        }

        // Ported from Artemis: remember the StreamView's original parent so it can be moved
        // back and forth when streaming to an external/secondary display (see showSecondScreen()).
        ...
        if (prefConfig.enablePerfOverlay) {
            ...
            setupPerfOverlayInteraction();
            startPerfOverlayExtraTicker();
            showPerfOverlay();
        }
        ...
        // Ported from Artemis: the experimental "Warp Drive" frame pacing options request a
        // multiple (2x/4x) of the actual target frame rate from the host. Some devices/decoders
        // exhibit smoother perceived pacing this way. framePacingWarpFactor defaults to 0
        // (treated as no-op below) unless the user opted into one of those two pacing options.
        ...
        if (!decoderRenderer.isHevcSupported() && !decoderRenderer.isAv1Supported()) {
            if (spinner != null) {
                spinner.dismiss();
                spinner = null;
            }

            // If we can't find an HEVC or AV1 decoder, we can't proceed
            Dialog.displayDialog(this, getResources().getString(R.string.conn_error_title),
                    "This device or ROM doesn't support hardware accelerated HEVC or AV1 playback.", true);
            return;
        }
        ...
        // Ported from Artemis: move the stream to a connected external/secondary display if
        // the user has enabled that mode and one is actually present.
        if (prefConfig.enableExDisplay) {
            showSecondScreen();
        }
    }

    // Ported from Artemis: finds the first non-default Display and moves the existing
    // StreamView into a Presentation shown on it, leaving virtual controls/overlays on the
    // device's own screen. Falls back to doing nothing if no secondary display is connected.
    private void showSecondScreen() {
        ...
        if (secondaryDisplayId != -1) {
            ...
            presentation.show();
            if (rootView != null) {
                ((ViewGroup) rootView).removeView(streamView);
                presentation.addView(streamView);
            }
            // Force mouse mode to trackpad since the user won't see the device's own screen anymore.
            applyMouseMode(2);
        }
    }
    ...
    private void setPreferredOrientationForCurrentDisplay() {
        ...
        if (PreferenceConfiguration.isSquarishScreen(display)) {
            ...
            // Ported from Artemis: on a regular (non-squarish) phone screen, opt-in users can
            // rotate to portrait via the game menu (rotateScreen()) instead of always being
            // locked to landscape below. Start out honoring whatever rotateScreen() last chose
            // (defaulting to landscape the first time), rather than re-deriving it here.
    ...
    // Public (not private) so PanZoomHandler (ported from Artemis, lives in the different
    // package com.limelight.utils) can call this after a pinch-zoom gesture ends.
    ...
    private float prepareDisplayForRendering() {
        ...
        if (prefConfig.videoScaleMode == PreferenceConfiguration.ScaleMode.STRETCH || aspectRatioMatch) {
            ...
            // Ported from Artemis: FILL crops to cover the whole screen (may overflow one
            // dimension) instead of letterboxing/pillarboxing like FIT does.
    ...
    protected void onDestroy() {
        ...
        // Ported from Artemis: dismiss the external display presentation, if any, to avoid
        // leaking its window.
        if (presentation != null) {
            presentation.dismiss();
        }

        if (perfOverlayTicker != null) {
            perfOverlayHandler.removeCallbacks(perfOverlayTicker);
        }
    ...
    public boolean handleKeyDown(KeyEvent event) {
        ...
        if (!handled) {
            ...
            if (translated == 0) {
                // Ported from Artemis: let a long-press-free physical BACK button send the
                // Meta/Windows key instead of being swallowed, when there's no other Unicode
                // representation for it.
                if (prefConfig.backAsMeta && event.getKeyCode() == KeyEvent.KEYCODE_BACK) {
                    translated = 0x5b; // VK_LWIN
                }
                else {
    ...
    public boolean handleKeyUp(KeyEvent event) {
        ...
        if (!handled) {
            ...
            short translated = keyboardTranslator.translate(event.getKeyCode(), event.getDeviceId());
            if (translated == 0) {
                // Ported from Artemis: mirror the key-down handling above so a BACK-as-Meta
                // press is reported as handled on key up too, instead of falling through to
                // the UTF-8 dead-character check below.
                if (prefConfig.backAsMeta && event.getKeyCode() == KeyEvent.KEYCODE_BACK) {
                    translated = 0x5b; // VK_LWIN
                }
                else {
    ...
    // Ported from Artemis: quit WITH a confirmation prompt, as a distinct option from
    // disconnect() above - kept separate so users can bind/reach whichever behavior they want
    // from the game menu.
    void quit() {
    ...
    // Ported from Artemis: toggles between landscape and portrait lock. Only meaningful when
    // prefConfig.autoOrientation is enabled - see setPreferredOrientationForCurrentDisplay().
    void rotateScreen() {
    ...
    // Ported from Artemis: while enabled, all touch input pans/pinch-zooms the stream view via
    // panZoomHandler instead of being sent to the host as touch/mouse/OSC input.
    boolean isZoomModeEnabled() {
    ...
    // Ported from Artemis: toggles whether the (already-grabbed) mouse pointer is visible while
    // in relative/trackpad mouse mode - was previously only reachable via a keyboard shortcut
    // (Ctrl+Alt+Shift+C, see handleSpecialKeys()), now also reachable from the Game Menu's mouse
    // mode selector for touch-only users.
    void toggleMouseLocalCursor() {
        if (!grabbedInput) {
            inputCaptureProvider.enableCapture();
            grabbedInput = true;
        }
    ...
    // Ported from Artemis: shows a list of touch/mouse input modes the user can switch to
    // mid-session (not persisted - always starts from whatever checkbox_touchscreen_trackpad/
    // absolute mouse mode settings say at connection time). Reachable from the Game Menu.
    void selectMouseMode() {
        String[] baseNames = getResources().getStringArray(R.array.mouse_mode_names);
        String[] items = java.util.Arrays.copyOf(baseNames, baseNames.length + 1);
        items[items.length - 1] = getString(R.string.game_menu_toggle_local_cursor);

        new AlertDialog.Builder(this).setItems(items, (dialog, which) -> {
            dialog.dismiss();
            if (which == baseNames.length) {
                toggleMouseLocalCursor();
                return;
            }
    ...
    // Ported from Artemis: reconfigures touchContextMap[] in place for the selected mouse mode.
    // Mode indices match res/values/arrays.xml's mouse_mode_values:
    // 0 = multi-touch, 1 = normal mouse, 2 = trackpad (natural), 3 = trackpad (gaming/relative),
    // 4 = disabled, 5 = normal mouse (swapped buttons).
    void applyMouseMode(int mode) {
        switch (mode) {
            case 0: // Multi-touch
            ...
            case 3: // Trackpad (gaming/relative)
                prefConfig.enableMultiTouchScreen = false;
        ...
        for (int i = 0; i < touchContextMap.length; i++) {
            ...
            if (mode == 4) {
                ...
                touchContextMap[i] = new com.limelight.binding.input.touch.TrackpadContext(conn, i,
                        prefConfig.trackpadSwapAxis, prefConfig.trackpadSensitivityX, prefConfig.trackpadSensitivityY);
            }
        }

        // Always exit pan/zoom mode if the mouse mode has changed, matching Artemis - avoids a
        // confusing state where touch input silently does nothing because it is still being
        // routed to the (now stale) pan/zoom handler instead of the newly selected mode.
        isPanZoomMode = false;
    }

    // Shows the draggable performance overlay container in whichever collapsed/expanded state
    // is currently active (used both for the initial show and when restoring after PiP).
    ...
    // treats a tap (movement below a small threshold) as a request to expand/collapse it -
    // the same interaction pattern used by Parsec's stats overlay.
    private void setupPerfOverlayInteraction() {
        final View container = findViewById(R.id.perfOverlayContainer);

        View.OnTouchListener dragListener = new View.OnTouchListener() {
            private float dX, dY;
            private float downRawX, downRawY;
            private static final float CLICK_DRAG_TOLERANCE = 20;
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getActionMasked()) {
                    case MotionEvent.ACTION_DOWN:
                        ...
                        dX = container.getX() - downRawX;
                        dY = container.getY() - downRawY;
                        return true;
                    ...
                    case MotionEvent.ACTION_UP:
                        ...
                        if (upDX < CLICK_DRAG_TOLERANCE && upDY < CLICK_DRAG_TOLERANCE) {
                            togglePerfOverlayExpanded();
                        }
                        return true;
                    default:
                        return false;
        ...
        findViewById(R.id.perfOverlayIcon).setOnTouchListener(dragListener);
        findViewById(R.id.perfOverlayPanel).setOnTouchListener(dragListener);
    }

    // Independent ~300ms ticker for the session timer + live mic level indicator, decoupled from
    // MediaCodecDecoderRenderer's own ~1s FPS/latency update cadence so the mic "VU meter" feels
    // reasonably responsive when the user talks.
    private void startPerfOverlayExtraTicker() {
        final TextView extraView = findViewById(R.id.perfOverlayExtra);

        perfOverlayTicker = new Runnable() {
            @Override
            public void run() {
                extraView.setText(buildExtraOverlayText());
                perfOverlayHandler.postDelayed(this, 300);
            }
        };
        perfOverlayHandler.post(perfOverlayTicker);
    }

    private String buildExtraOverlayText() {
        long elapsedSec = Math.max(0, (System.currentTimeMillis() - sessionStartTimeMs) / 1000);
        String sessionTime = String.format(Locale.getDefault(), "%02d:%02d:%02d",
                elapsedSec / 3600, (elapsedSec % 3600) / 60, elapsedSec % 60);

        StringBuilder sb = new StringBuilder();
        sb.append(getString(R.string.perf_overlay_session_time, sessionTime));

        if (micForwarder != null && micForwarder.isRunning()) {
            int bars = Math.round(micForwarder.getCurrentLevel() * 10);
            StringBuilder barBuilder = new StringBuilder();
            for (int i = 0; i < 10; i++) {
                barBuilder.append(i < bars ? '\u2588' : '\u2591');
            }
            sb.append('\n').append(getString(R.string.perf_overlay_mic_active, barBuilder.toString()));
        }
        else {
            sb.append('\n').append(getString(R.string.perf_overlay_mic_inactive));
        }

        return sb.toString();
    }

    // VK_* codes (Windows Virtual-Key codes) for keys the default soft keyboard can't easily
    // produce (Ctrl, Alt, the Windows key, Ctrl+Alt+Del, etc.) - used by the Special Keys menu.
    private static final short VK_LSHIFT = 0xA0;
    private static final short VK_LCONTROL = 0xA2;
    private static final short VK_LMENU = 0xA4; // Left Alt
    private static final short VK_LWIN = 0x5B; // Windows key
    private static final short VK_TAB = 0x09;
    private static final short VK_ESCAPE = 0x1B;
    private static final short VK_DELETE = 0x2E;
    private static final short VK_F4 = 0x73;
    private static final short VK_C = 0x43;
    private static final short VK_V = 0x56;
    private static final short VK_Z = 0x5A;

    private static final class SpecialKeyAction {
        final String label;
        final short[] keys;

        SpecialKeyAction(String label, short... keys) {
            this.label = label;
            this.keys = keys;
        }
    }

    private static final SpecialKeyAction[] SPECIAL_KEYS = {
            new SpecialKeyAction("Esc", VK_ESCAPE),
            new SpecialKeyAction("Tab", VK_TAB),
            new SpecialKeyAction("Win", VK_LWIN),
            new SpecialKeyAction("Ctrl+Alt+Del", VK_LCONTROL, VK_LMENU, VK_DELETE),
            new SpecialKeyAction("Alt+Tab", VK_LMENU, VK_TAB),
            new SpecialKeyAction("Alt+F4", VK_LMENU, VK_F4),
            new SpecialKeyAction("Ctrl+C", VK_LCONTROL, VK_C),
            new SpecialKeyAction("Ctrl+V", VK_LCONTROL, VK_V),
            new SpecialKeyAction("Ctrl+Z", VK_LCONTROL, VK_Z),
            new SpecialKeyAction("Ctrl+Shift+Esc", VK_LCONTROL, VK_LSHIFT, VK_ESCAPE),
    };

    // Sends a synthetic key combination (e.g. Ctrl+Alt+Del) straight to the host, independent of
    // any real KeyEvent - used by the in-stream Special Keys menu below.
    private void sendSyntheticKeyCombo(short[] vkCodes) {
        if (conn == null) {
            return;
        }

        byte comboModifier = 0;
        for (short vk : vkCodes) {
            if (vk == VK_LCONTROL) {
                comboModifier |= KeyboardPacket.MODIFIER_CTRL;
            }
            else if (vk == VK_LMENU) {
                comboModifier |= KeyboardPacket.MODIFIER_ALT;
            }
            else if (vk == VK_LSHIFT) {
                comboModifier |= KeyboardPacket.MODIFIER_SHIFT;
            }
            else if (vk == VK_LWIN) {
                comboModifier |= KeyboardPacket.MODIFIER_META;
            }
        }

        for (short vk : vkCodes) {
            conn.sendKeyboardInput(vk, KeyboardPacket.KEY_DOWN, comboModifier, (byte) 0);
        }
        for (int i = vkCodes.length - 1; i >= 0; i--) {
            conn.sendKeyboardInput(vkCodes[i], KeyboardPacket.KEY_UP, comboModifier, (byte) 0);
        }
    }

    // Shows the in-stream Special Keys menu - lets the user send key combos the default soft
    // keyboard can't (Ctrl, Alt, Win, Ctrl+Alt+Del, etc.) and jump to the system input method
    // picker to switch to a PC-style keyboard app like Hacker's Keyboard.
    // Package-private (not private) so GameMenu (same com.limelight package) can reuse this
    // instead of duplicating special-key-sending logic.
    void showSpecialKeysMenu() {
        View view = LayoutInflater.from(this).inflate(R.layout.dialog_special_keys, null);

        Button chooseKeyboardButton = view.findViewById(R.id.chooseKeyboardButton);
        ...
        final Button micForwardButton = view.findViewById(R.id.micForwardButton);
        micForwardButton.setText(micForwarder != null && micForwarder.isRunning() ?
                R.string.game_mic_forward_on : R.string.game_mic_forward_off);
        micForwardButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (micForwarder != null && micForwarder.isRunning()) {
                    micForwarder.stop();
                    micForwardButton.setText(R.string.game_mic_forward_off);
                    Toast.makeText(Game.this, R.string.game_mic_forward_stopped, Toast.LENGTH_SHORT).show();
                }
                else if (checkSelfPermission(Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED) {
                    startMicForwarding();
                    micForwardButton.setText(R.string.game_mic_forward_on);
                }
                else {
        ...
        GridLayout grid = view.findViewById(R.id.specialKeysGrid);
        float density = getResources().getDisplayMetrics().density;
        for (final SpecialKeyAction action : SPECIAL_KEYS) {
            Button keyButton = new Button(this);
            keyButton.setText(action.label);
            keyButton.setTextColor(getResources().getColor(R.color.moonlight_gold_foreground));
            keyButton.setBackgroundResource(R.drawable.bg_button_purple);
            keyButton.setAllCaps(false);
            keyButton.setTextSize(13);
            keyButton.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    sendSyntheticKeyCombo(action.keys);
                }
            });
    ...
    private void startMicForwarding() {
        if (hostAddress == null) {
            return;
        }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == MIC_PERMISSION_REQUEST_CODE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(this, R.string.game_mic_forward_denied, Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    private boolean handleMotionEvent(View view, MotionEvent event) {
                 ...
                 eventSource == InputDevice.SOURCE_MOUSE_RELATIVE)
        {
            ...
            else
            {
                ...
                // Ported from Artemis: while pan/zoom mode is on, all touch input pans/pinch-
                // zooms the stream view instead of being sent to the host.
                if (isPanZoomMode) {
                ...
                // Ported from Artemis: when multi-touch mode is selected via the Game Menu,
                // skip the 3-finger-tap-for-keyboard special case entirely (the host is
                // receiving raw per-finger touch points directly in that mode, so this
                // synthetic gesture wouldn't make sense) and forward touch events natively
                // instead of going through the touchContextMap dispatch below.
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ...
        // Ported from Artemis: keep the pan/zoom handler's cached dimensions in sync whenever
        // the surface's size changes (e.g. rotation, resizing in multi-window mode).
