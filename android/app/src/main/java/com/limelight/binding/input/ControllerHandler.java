...
public class ControllerHandler implements InputManager.InputDeviceListener, UsbDriverListener {
    ...
    private InputDeviceContext createInputDeviceContextForDevice(InputDevice dev) {
        ...
        if (prefConfig.enableDeviceRumble) {
            // Ported from Artemis: force use of the device's own vibration motor instead of
            // the gamepad's, for gamepads whose own rumble motor isn't effective/present.
            context.vibrator = deviceVibrator;
        }
        else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S && hasQuadAmplitudeControlledRumbleVibrators(dev.getVibratorManager())) {
    ...
    private int handleRemapping(InputDeviceContext context, KeyEvent event) {
        ...
        // Ported from Artemis: fix D-pad/face button scan code mapping for standalone
        // JoyCon-Left and JoyCon-Right controllers (vendorId 0x057e), which some kernels
        // report with raw scan codes that don't line up with the standard mapping above.
        ...
        if (prefConfig.enableJoyConFix && context.vendorId == 0x057e && context.productId == 0x2007) {
            // JoyCon-Right face buttons (X/Y swapped on some kernels)
            switch (event.getScanCode()) {
                case 307:
                    return KeyEvent.KEYCODE_BUTTON_Y;
                case 308:
