...
public class UiHelper {
    ...
    private static final String THEME_PREF_STRING = "list_app_theme";

    // Applies the user's selected appearance (Settings > Appearance) to this Activity. Must be
    // called before super.onCreate() so the theme is active before the window is created.
    public static void applyPreferredTheme(Activity activity) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(activity);
        String theme = prefs.getString(THEME_PREF_STRING, "dark");
    ...
    // Resolves a custom theme color attribute (e.g. R.attr.appForegroundColor) against the
    // current theme of the given context. Used to tint plain vector icons (like ic_computer) at
    // runtime so they follow the active Dark/Light appearance without relying on ?attr
    // references inside vector drawable XML (unreliable to build/inflate on older API levels).
