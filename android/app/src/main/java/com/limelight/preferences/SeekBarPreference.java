...
public class SeekBarPreference extends DialogPreference
{
    ...
    protected View onCreateDialogView() {
        ...
        if (shouldPersist()) {
            currentValue = getPersistedIntSafe(defaultValue);
        }
    ...
    protected void onSetInitialValue(boolean restore, Object defaultValue)
    {
        ...
        if (restore) {
            currentValue = shouldPersist() ? getPersistedIntSafe(this.defaultValue) : 0;
        }
        else {
    ...
    // A key previously used by a differently-typed preference (e.g. a String-backed
    // ListPreference, as "seekbar_bitrate_kbps" was before this control replaced it) can leave
    // behind a value of the wrong type under the same SharedPreferences key, which would
    // otherwise make getPersistedInt() throw a ClassCastException. Treat that case as "no
    // valid value yet" rather than crashing.
    private int getPersistedIntSafe(int defaultValue) {
        try {
    ...
    // attributes alone since it can depend on runtime information not known at inflate time.
