...
public class DiskAssetLoader {
    ...
    public ScaledBitmap loadBitmapFromCache(CachedAppAssetLoader.LoaderTuple tuple, int sampleSize) {
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.P) {
            ...
            // On P, we can get a bitmap back in one step with ImageDecoder. This is isolated
            // in ImageDecoderCompat so this class never needs to resolve ImageDecoder types on
            // devices older than P (see comment on ImageDecoderCompat for why that matters).
            try {
                return ImageDecoderCompat.decode(file, STANDARD_ASSET_WIDTH, STANDARD_ASSET_HEIGHT, isLowRamDevice);
            } catch (IOException e) {
                e.printStackTrace();
                return null;
            }
