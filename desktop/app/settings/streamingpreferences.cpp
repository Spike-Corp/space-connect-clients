...
#define SER_MICFORWARDING "micforwarding"
...
void StreamingPreferences::reload()
{
    ...
    videoCodecConfig = static_cast<VideoCodecConfig>(settings.value(SER_VIDEOCFG,
                                                  static_cast<int>(VideoCodecConfig::VCC_FORCE_HEVC)).toInt());
    ...
    // "Space Connect" defaults to Brazilian Portuguese rather than LANG_AUTO - Qt's
    // QLocale::system() reflects the OS's primary UI language, which doesn't always
    // match a user's actual preferred language (e.g. regional format set to pt_BR
    // while the Windows display language itself is still English). Automatic is
    // still available as a language option for users who want OS-driven detection.
    language = static_cast<Language>(settings.value(SER_LANGUAGE,
                                                    static_cast<int>(Language::LANG_PT_BR)).toInt());
...
void StreamingPreferences::save()
{
    ...
    settings.setValue(SER_MICFORWARDING, enableMicForwarding);
    settings.setValue(SER_MIC_DEVICE, micCaptureDeviceName);
}
