...
void ComputerManager::startPolling()
{
    ...
    // mDNS/local network autodiscovery is intentionally hard-disabled - this client only ever
    // connects to the user's own cloud VM (provisioned via spacecloud.gg), never to a PC on the
    // local network, so we never want to identify/list any local GameStream/Sunshine hosts a
    // user might happen to have on their LAN. This is NOT gated behind m_Prefs->enableMdns
    // (which some existing installs may still have saved as true from before) - it must never
    // run regardless of any previously-persisted preference value.
    qWarning() << "mDNS local network discovery is disabled";
...
QString ComputerManager::getPrimaryGpuModel()
{
    QVector<NvComputer*> hosts = getComputers();
    if (hosts.isEmpty()) {
        return QString();
    }

    NvComputer* computer = hosts.first();
    ...
    void run()
    {
        ...
        try {
           ...
           case NvPairingManager::PairState::PAIRED:
            ...
            emit pairingCompleted(m_Computer, tr("GameStream returned error: %1").arg(e.toQString()));
