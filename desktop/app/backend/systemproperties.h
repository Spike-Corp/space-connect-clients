...
public:
    ...
    // Enumerates the names of available audio RECORDING (capture/microphone) devices for use
    // in a settings picker - e.g. for microphone forwarding input device selection. Does not
    // include an "Automatic"/default entry; callers should add that themselves if desired.
    Q_INVOKABLE QStringList getAudioCaptureDeviceNames();
