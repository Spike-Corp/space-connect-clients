...
bool Session::validateLaunch(SDL_Window* testWindow)
{
    if (!m_Computer->isSupportedServerVersion) {
        emit displayLaunchError(tr("The version of GameStream on %1 is not supported by this build of Space Connect. You must update Space Connect to stream from %1.").arg(m_Computer->name));
        return false;
    }
    ...
    if (m_StreamConfig.width >= 3840) {
        ...
        if (m_Computer->gfeVersion.isEmpty() || m_Computer->gfeVersion.startsWith("2.")) {
            emitLaunchWarning(tr("GameStream 3.0 or higher is required for 4K streaming."));
    ...
    if (!SdlInputHandler::getUnmappedGamepads().isEmpty()) {
        emitLaunchWarning(tr("An attached gamepad has no mapping and won't be usable. Visit the Space Connect help to resolve this."));
    }
...
#else
    std::string windowName = QString(m_Computer->name + " - Space Connect").toStdString();
#endif
...
DispatchDeferredCleanup:
    // Stop forwarding the local microphone before anything else - no ordering
    // dependency with the input handler/decoder cleanup below.
    if (m_MicForwarder != nullptr) {
        m_MicForwarder->stopForwarding();
        delete m_MicForwarder;
        m_MicForwarder = nullptr;
    }
