...
macx:!disable-prebuilts {
    ...
    # Note: $$shell_quote() is required here (not just embedding the raw path) because this
    # repo's checkout path can contain spaces - an unquoted path breaks word-splitting when the
    # generated Makefile recipe line is run through /bin/sh, causing clang to see a truncated
    # path (see repo memory notes on this exact class of bug).
    QMAKE_CFLAGS += -F$$shell_quote($$PWD/../libs/mac/Frameworks)
    QMAKE_CXXFLAGS += -F$$shell_quote($$PWD/../libs/mac/Frameworks)
    QMAKE_OBJECTIVE_CFLAGS += -F$$shell_quote($$PWD/../libs/mac/Frameworks)
}
...
win32 {
    ...
    QMAKE_TARGET_COMPANY = Space Connect
    QMAKE_TARGET_DESCRIPTION = Space Connect Streaming Client
    QMAKE_TARGET_PRODUCT = Space Connect
