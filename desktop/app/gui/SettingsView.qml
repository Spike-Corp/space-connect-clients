        ...
        GroupBox {
            ...
            Column {
                ...
                Row {
                    ...
                    Image {
                        source: "qrc:/res/icons/device-desktop.svg"
                        width: 16
                        height: 16
                    ...
                    SpaceComboBox {
                        ...
                        function updateBitrateForSelection() {
                            ...
                            if (StreamingPreferences.width !== selectedWidth || StreamingPreferences.height !== selectedHeight) {
                                ...
                                bitrateSpinBox.syncFromPreferences()
                            }
                        ...
                        NavigableDialog {
                            ...
                            ColumnLayout {
                                Label {
                                    text: qsTr("Custom resolutions are not officially supported by GameStream, so it will not set your host display resolution. You will need to set it manually while in game.") + "\n\n" +
                ...
                Row {
                    ...
                    Image {
                        source: "qrc:/res/icons/gauge.svg"
                        width: 16
                        height: 16
                        sourceSize: Qt.size(16, 16)
                        anchors.verticalCenter: parent.verticalCenter
                ...
                SpaceSpinBox {
                    ...
                    // "physical machine" tier hardware. Word boundaries (\b) keep this from false-
                    // matching similarly-named but different GPUs, e.g. "A10" must not match "A100",
                    // and "L4" must not match "L40"/"L40S".
                    function isHighTierGpu(gpuModel) {
                        return !!gpuModel && /\b(L4|A10|RTX 3080 Ti)\b/i.test(gpuModel)
                    }
                    ...
                    Component.onCompleted: {
                        ...
                        // state is refreshed so the 50 Mbps cap can unlock (or, if switching
                        // to a lower-tier host, re-clamp) without needing to close/reopen
                        // Settings.
                        ComputerManager.computerStateChanged.connect(function() {
                            updateMaxBitrate()
                            syncFromPreferences()
                        })
                    }
        ...
        GroupBox {
            ...
            Column {
                ...
                SpaceCheckBox {
                    ...
                    width: parent.width
                    text: qsTr("Mute audio stream when Space Connect is not the active window")
                    font.pointSize: 12
                    visible: SystemProperties.hasDesktopEnvironment
                    ...
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Mutes Space Connect's audio when you Alt+Tab out of the stream or click on a different window.")
                }
                ...
                Row {
                    ...
                    Image {
                        source: "qrc:/res/icons/microphone.svg"
                        width: 16
                        height: 16
                        sourceSize: Qt.size(16, 16)
                        anchors.verticalCenter: micForwardingCheck.verticalCenter
                    }

                    SpaceCheckBox {
                        id: micForwardingCheck
                        width: parent.width - 22
                        text: qsTr("Forward this PC's microphone to the host (Experimental)")
                        font.pointSize: 12
                        checked: StreamingPreferences.enableMicForwarding
                        onCheckedChanged: {
                            StreamingPreferences.enableMicForwarding = checked
                        }

                        ToolTip.delay: 1000
                        ToolTip.timeout: 10000
                        ToolTip.visible: hovered
                        ToolTip.text: qsTr("Sends audio from your microphone to a virtual microphone on the host PC, so it can be picked up by voice chat or other apps running there.") + "\n\n" +
                                      qsTr("Requires the \"Space Connect Mic Bridge\" companion program (and a virtual audio cable) installed and running on the host PC.")
                    }
                }
                ...
                Label {
                    width: parent.width
                    id: micDeviceTitle
                    text: qsTr("Microphone input device")
                    font.pointSize: 12
                    wrapMode: Text.Wrap
                    enabled: micForwardingCheck.checked
                }
                ...
                SpaceComboBox {
                    ...
                    // Picking the wrong input device (e.g. a webcam mic instead of a headset
                    // mic) is a common cause of "mic forwarding does nothing" - letting the
                    // user explicitly choose avoids relying on whatever the OS considers the
                    // current default recording device.
                    function rebuildModel() {
                        var savedDevice = StreamingPreferences.micCaptureDeviceName

                        micDeviceListModel.clear()
                        micDeviceListModel.append({"text": qsTr("Automatic (Recommended)"), "deviceName": ""})

                        var devices = SystemProperties.getAudioCaptureDeviceNames()
                        for (var i = 0; i < devices.length; i++) {
                            micDeviceListModel.append({"text": devices[i], "deviceName": devices[i]})
                        }

                        currentIndex = 0
                        for (var j = 0; j < micDeviceListModel.count; j++) {
                            if (micDeviceListModel.get(j).deviceName === savedDevice) {
                                currentIndex = j
                                break
                            }
                        }
                    ...
                    onActivated: {
                        StreamingPreferences.micCaptureDeviceName = micDeviceListModel.get(currentIndex).deviceName
                    }

                    Component.onCompleted: {
                        rebuildModel()
                        languageChanged.connect(rebuildModel)
                    }
                    ...
                    onActivated : {
                        ...
                        if (StreamingPreferences.language !== new_language) {
                            ...
                            if (!StreamingPreferences.retranslate()) {
                                ToolTip.show(qsTr("You must restart Space Connect for this change to take effect"), 5000)
                            }
                            else {
            ...
            Column {
                ...
                SpaceCheckBox {
                    ...
                    ToolTip.text: qsTr("This enables seamless mouse control without capturing the client's mouse cursor. It is ideal for remote desktop usage but will not work in most games.") + " " +
                                  ...
                                  qsTr("NOTE: Due to a bug in GameStream, this option may not work properly if your host PC has multiple monitors.")
                }
                ...
                Row {
                    ...
                    SpaceCheckBox {
                        ...
                        ToolTip.text: qsTr("This enables the capture of system-wide keyboard shortcuts like Alt+Tab that would normally be handled by the client OS while streaming.") + "\n\n" +
                                      qsTr("NOTE: Certain keyboard shortcuts like Ctrl+Alt+Del on Windows cannot be intercepted by any application, including Space Connect.")
                    }
        ...
        GroupBox {
            ...
            Column {
                ...
                SpaceCheckBox {
                    ...
                    width: parent.width
                    text: qsTr("Process gamepad input when Space Connect is in the background")
                    font.pointSize: 12
                    visible: SystemProperties.hasDesktopEnvironment
                    ...
                    ToolTip.timeout: 5000
                    ToolTip.visible: hovered
                    ToolTip.text: qsTr("Allows Space Connect to capture gamepad inputs even if it's not the current window in focus")
                }
            }
        }
        ...
        GroupBox {
            ...
            Column {
                ...
                Label {
                    width: parent.width
                    id: resVCCdesc
                    text: qsTr("HEVC runs normally on our current T4 hosts. AV1 requires a newer GPU (L4, RTX 3080 Ti, or A10) and may not be available on older hardware.")
                    font.pointSize: 9
                    wrapMode: Text.Wrap
                }
                ...
                SpaceComboBox {
                    ...
                    Component.onCompleted: {
                        ...
                        // Default to HEVC (relevant if HDR is enabled or the saved value is an
                        // old/removed option like Automatic or H.264, where we will match none
