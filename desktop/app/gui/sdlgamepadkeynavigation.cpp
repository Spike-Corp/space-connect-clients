...
{
    if (!m_Enabled) {
        // The original assumption here was that PcView.qml's StackView.onActivated (which
        // calls enable() before this) always runs first and is never destroyed, so m_Enabled
        // would always be true by the time any other view (AppView/SettingsView) calls this.
        // That assumption doesn't hold in this fork: LoginView.qml can be the very first view
        // activated, and its toolbar lets the user reach SettingsView (or, via a direct-launch
        // app, AppView) before PcView ever activates and enables the SDL gamepad subsystem.
        // Treat "not enabled yet" as "no gamepads connected" instead of asserting/crashing -
        // SDL_NumJoysticks()/SDL_IsGameController() also aren't safe to call before
        // SDL_InitSubSystem(SDL_INIT_GAMECONTROLLER) has run in enable().
        return 0;
    }
